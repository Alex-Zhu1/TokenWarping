...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    former_frame_index = torch.arange(video_lengths) - 1
    former_frame_index[0] = 0
    flownet.eval()
    # 添加flownet,得到flow
    with torch.no_grad():
        rgb_img = con.to(device).float()
        target_frame = rearrange(rgb_img, 'f h w c -> f c h w') # f h w c - f c h w
        source_frame = target_frame[former_frame_index, ...]  # i - 1
        if_pixel_occ = False
        ...
        flow = torch.cat((bwd_flow.to(weight_dtype), mask.to(weight_dtype)), dim=1)
    ...
    indices = [list(range(i, min(i+8, video_lengths))) for i in range(0, video_lengths, 8)]
    ...
    for batch_ind, indice in enumerate(indices):
        ...
        if validation_data.edit_type == 'DDIM':
            for idx, prompt in enumerate(validation_data.prompts):
                infer_sample = validation_pipeline(prompt + POS_PROMPT,
                                                   generator=generator, latents=ddim_inv_latent,
                                                   image=control_i, negative_prompt=NEG_PROMPT,
        ...
        samples.append(sample_batch[0])

        gc.collect()
        torch.cuda.empty_cache()

    print("save translated video")
    samples = torch.cat(samples, dim=2)   # b c f h w 进行warp
