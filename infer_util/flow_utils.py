...
def bilinear_sample(img,
    ...
    grid = torch.stack([x_grid, y_grid], dim=-1).to(img.dtype)  # [B, H, W, 2]
...
@torch.no_grad()
def get_flow_and_occ(flow_model,
                     image1,
                     image2,
                     if_pixel_occ=False):
    B = image1.shape[0]
    ...
    results_dict = flow_model(image1,
                              ...
                              prop_radius_list=[-1],
                              pred_bidir_flow=True)
    flow_pr = results_dict['flow_preds'][-1]  # [B, 2, H, W]  16 2 H W
    fwd_flow = padder.unpad(flow_pr[:B])  # [1, 2, H, W]
    bwd_flow = padder.unpad(flow_pr[B:])  # [1, 2, H, W]
    fwd_occ, bwd_occ = forward_backward_consistency_check(
        fwd_flow, bwd_flow)  # [1, H, W] float
    if if_pixel_occ:
        # 精细的mask
        warped_image1 = flow_warp(image1, bwd_flow)
        bwd_occ = torch.clamp(bwd_occ + (abs(image2-warped_image1).mean(dim=1)>255*0.25).float(), 0 ,1)
    return bwd_occ.unsqueeze(1), bwd_flow


class FlowCalc():

    def __init__(self, model_path='./deps/models/gmflow_sintel-0c07dcb3.pth'):
        flow_model = GMFlow(
