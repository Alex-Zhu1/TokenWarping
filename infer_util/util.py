...
def save_tensor_images_folder(videos: torch.Tensor, path: str, rescale=False, n_rows=4):
    ...
    # videos = rearrange(videos, "t b c d h w -> t b (c d) h w")
    outputs = []
...
def get_noise_pred_single(latents, t, context, unet, controlnet, controls, controlnet_conditioning_scale=1.0, flow=None):
...
@torch.no_grad()
def ddim_loop(pipeline, ddim_scheduler, latent, num_inv_steps, prompt,controls, controlnet_conditioning_scale, flow):
    ...
    for i in tqdm(range(num_inv_steps)):
        ...
        noise_pred = get_noise_pred_single(latent, t, cond_embeddings, pipeline.unet, pipeline.controlnet, controls, controlnet_conditioning_scale, flow)
...
@torch.no_grad()
def ddim_inversion(pipeline, ddim_scheduler, video_latent, num_inv_steps, prompt="", controls=None, controlnet_conditioning_scale=1.0, flow=None):
    ddim_latents = ddim_loop(pipeline, ddim_scheduler, video_latent, num_inv_steps, prompt, controls, controlnet_conditioning_scale, flow)
    return ddim_latents
